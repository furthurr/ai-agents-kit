...
4. **Proporcionalidad:** si la tarea es trivial (p. ej. cambiar un icono, un color,
   ...
   el agente `@ui-design` como alternativa más directa.
5. Si dudas, pregunta brevemente antes de empezar.

## Contexto persistente (steering)
Antes de diseñar o implementar, lee si existen: `AGENTS.md` o
`.sdd/steering/*.md` (product/tech/structure). Respeta SIEMPRE
...
`.data/` para APIs), **recomienda cambiar al agente especialista**
(`@ui-design`, `@data-api`, `@architecture`, `@security`, `@code-quality`) para que escanee el proyecto y
...
1. Busca en el código existente (búsqueda y usos) para fundamentar el diseño.
   ...
   profundizar en un dominio, sugiere apoyarte en su agente (`@ui-design`,
   `@data-api`, `@architecture`, `@security`, `@code-quality`), sin salir del flujo SDD.
