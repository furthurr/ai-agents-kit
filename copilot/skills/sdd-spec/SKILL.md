...
especificaciones de alta calidad y trazables. Complementa al agente personalizado
`SDD (Spec-Driven Development)` y funciona con cualquier agente de Copilot.
...
1. **Lanza el flujo.** Selecciona el agente `SDD` o escribe `/sdd-spec` y describe
...
> **No avances de fase sin la aprobación explícita del usuario.** Como Copilot no
> tiene una herramienta de "pregunta" dedicada, el gate se implementa de forma
> natural: **termina tu turno con la pregunta y espera** la respuesta del
> usuario. Excepción: Quick Plan (sin gates).

### Fase 1 — Requirements
1. Lee el steering si existe (`.github/copilot-instructions.md`, `AGENTS.md`,
...
- Escribe `/sdd-spec` en el chat para invocarla como slash command, o
  simplemente describe tu feature/bug y Copilot la cargará por relevancia.
- Combínala con el agente personalizado `SDD (Spec-Driven Development)` para el
  flujo completo con sus herramientas.
