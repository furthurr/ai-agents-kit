...
especificaciones de alta calidad y trazables. Complementa al agente {{sdd_agent}} y funciona con cualquier agente de {{platform_name}}.
...
> `{{sdd_agent}}` y esta skill divergen, **manda esta skill**.
...
1. **Lanza el flujo.** {{sdd_start_instruction}} y describe
...
> **No avances de fase sin la aprobación explícita del usuario.** {{gate_instruction}} **termina tu turno con la pregunta y espera** la respuesta del
> usuario. Excepción: Quick Plan (sin gates).

### Fase 1 — Requirements
1. Lee el steering si existe ({{steering_paths}}, `.sdd/steering/*.md`). Detecta
